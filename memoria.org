...
($\text{dim}(S) < m$) definimos la matriz $A \in \mathcal{M}_{mxn}$
como la matriz que contiene en sus columnas los vectores de la base
de $S$.
...
$A^t\left(b - Ax\right) = \vec{0} \Rightarrow A^t b = A^t Ax$
